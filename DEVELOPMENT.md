...
│   │           └── kd-cosmic/   # ← 你在这里
│   │               ├── src/qdata_adapter_kd_cosmic/    # 适配器源码
│   │               ├── tests/               # 测试
│   │               └── api-docs/            # API 文档
...
KdCosmicAdapter 是连接 **kd-cosmic** 平台与 QDataV2 数据集成平台的桥梁：
...
- **数据同步**: 将 kd-cosmic 数据实时/定时同步到 QDataV2
- **数据写入**: 将 QDataV2 处理后的数据写回 kd-cosmic
- **流程触发**: 支持 kd-cosmic 事件触发 QDataV2 流程
...
```
kd-cosmic/
├── src/qdata_adapter_kd_cosmic/           # 源码
│   ├── __init__.py              # 导出 KdCosmicAdapter
│   ├── adapter.py               # 主适配器类
│   ├── exceptions.py            # 自定义异常
│   ├── auth.py                  # 认证处理
│   ├── constants.py             # 常量
...
```bash
# 进入项目目录
cd /path/to/qdatav2-server/apps/app_integration/scaffold/templates/adapter/kd-cosmic
...
# 带覆盖率
pytest --cov=qdata_adapter_kd_cosmic --cov-report=html
...
| 接口 | 用途 | 认证 | 适用场景 |
|------|------|------|---------|
| `standard` | 主接口 | OAuth2 | 常规数据操作 |
| `enterprise` | 备用接口 | HMAC | 特殊业务/大数据量 |

接口切换：

```python
# 使用主接口
context.settings = {"interface": "standard"}
adapter = KdCosmicAdapter(context)

# 使用备用接口
context.settings = {"interface": "enterprise"}
adapter = KdCosmicAdapter(context)
```
