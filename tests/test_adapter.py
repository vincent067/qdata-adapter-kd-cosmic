...
class TestKdCosmicAdapter:
    ...
    @pytest.mark.asyncio
    async def test_enterprise_interface_routing(
        self,
        enterprise_context: ConnectorContext,
        mock_token_cache: Any,
    ) -> None:
        """测试 enterprise 接口路由"""
        adapter = KdCosmicAdapter(enterprise_context, mock_token_cache)
        info = adapter.get_interface_info()

        assert info["interface_name"] == "enterprise"
        assert "enterprise" in info["available_interfaces"]

    @pytest.mark.asyncio
    async def test_enterprise_authenticate(
        self,
        enterprise_context: ConnectorContext,
        mock_token_cache: Any,
        httpx_mock: HTTPXMock,
        ...
        """测试 enterprise 接口认证"""
        # enterprise 接口可能使用不同的认证方式
        httpx_mock.add_response(
            method="POST",
            url="https://api-alt.example.com/gateway",
            json={
                "response": {
                    "code": 0,
