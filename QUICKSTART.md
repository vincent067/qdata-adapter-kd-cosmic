# KdCosmicAdapter 快速开始

> 5 分钟上手 kd-cosmic 适配器
...
# 调用平台特有 API
result = await adapter.invoke(
    method="kd_cosmic.goods.batchupdateflag",
    object_type="goods",
...
# 运行特定测试
pytest tests/test_adapter.py::TestKdCosmicAdapter::test_authenticate -v
...
|------|-------------------|---------------------|
| 认证方式 | OAuth2 / API Key | HMAC 签名 |
| 适用场景 | 标准 REST API | 专用网关/开放平台 |
| 性能 | 一般 | 针对大批量优化 |
...
**遇到问题？** 请提交 [GitHub Issue](https://github.com/qeasy/qdata-adapter-kd-cosmic/issues)